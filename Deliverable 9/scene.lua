...
function scene:init( _sceneNum )
	...
	if(_sceneNum == 1) then
	    ...
	    for i = 1, 25000 do
	      ...
	      scene[i].x = math.random(-display.contentWidth * 15, display.contentWidth * 15);
	      scene[i].y = math.random(-display.contentHeight * 15, display.contentHeight * 15);
		...
		camera:setParallax(1, 1, 0.8, 0.6, 0.5, 0.4, 0.3, 0.2) -- Here we set parallax for each layer in desceding order
		camera.damping = 3;
	end
end
